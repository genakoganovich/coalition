...
function formatDuration (secondes)
{
	...
	if (days > 0)	
		return days + " d " + zeroPad(hours,2) + " h " + zeroPad(minutes,2) + " m " + zeroPad(secondes,2) + " s";
	if (hours > 0)	
		return zeroPad(hours,2) + " h " + zeroPad(minutes,2) + " m " + zeroPad(secondes,2) + " s";
	if (minutes > 0)	
		return zeroPad(minutes,2) + " m " + zeroPad(secondes,2) + " s";
	return secondes + " s"; /* w//o leading zero*/
}
...
function addSumSimpleWorkers (inputs)
{
	return "<td class='headerCell'>" + inputs.length + " workers</td>";
}
...
function renderJobs ()
{
	...
	for (i=0; i < jobs.length; i++)
	{
		...
		//addTD (job.Order);
		addTD (job.ID);
		table += "<td style=\"text-align: left;\">&nbsp;" + job.Title + "</td>\n";
		...
		addTD (progress);
		//addTD (job.Affinity);
		table += "<td style=\"text-align: left;\">" + job.Affinity + "</td>\n";
		
		addTD (job.TimeOut);
		//addTD (job.Worker);
		table += "<td style=\"text-align: left;\">" + job.Worker + "</td>\n";
...
function renderWorkers ()
{
	...
	workers.sort (_sort);
	ShowTotalWorkers = str(workers.length);
...
function addjob ()
{
    var _data = {
        ...
        url:$('#url').attr("value"),
        maxWorkers:$('#maxWorkers').attr("value"),
        parent:viewJob
    };
