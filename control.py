import sys, getopt, urllib.parse, http.client, re
...
if action=="add":
	params = urllib.parse.urlencode({'parent':parent, 'title':title, 'cmd':cmd, 'dir':dir, 'priority':priority, 'retry':retry, 'timeout':timeout, 'affinity':affinity, 'dependencies':dependencies, 'localprogress':localprogress, 'globalprogress':globalprogress})
	...
	conn = http.client.HTTPConnection(re.sub("^http://", "", serverUrl))
	conn.request("POST", "/json/getjobs", params, {"Content-type": "application/x-www-form-urlencoded"})
	response = conn.getresponse()
	data = response.read()
	conn.close()
	...
	if id<0: 
		...
		conn = http.client.HTTPConnection(re.sub("^http://", "", serverUrl))
		conn.request("POST", "/json/clearjobs", params, {"Content-type": "application/x-www-form-urlencoded"})
		response = conn.getresponse()
		data = response.read()
		conn.close()
else:
